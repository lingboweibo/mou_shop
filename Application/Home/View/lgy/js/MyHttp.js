...
function MyHttp(success,error){
	...
	//apiName 参数表示接口名称
	this.send    = function(parameter,apiName){
		var parmeterName;
		var body = '';
		if(apiName != 'register' && apiName != 'login'){//判断如果不是注册接口或登录接口就加入公共参数
		...
		self.http.open('POST','http://124.172.137.59:8021/i.php?m=Api&c=Api&a=' + apiName);
